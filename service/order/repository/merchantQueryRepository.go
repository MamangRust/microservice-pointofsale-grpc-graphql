...
// wraps ONLY the raw client call so transport-level failures (Unavailable,
// DeadlineExceeded) reach isTransportFailure and trip the circuit breaker; the
// conversion to a business AppError happens after the guard returns, so the
// service error contract is unchanged.
