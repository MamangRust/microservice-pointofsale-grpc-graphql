...
func (s *RoleGraphqlTestSuite) gql(query string) map[string]interface{} {
...
func (s *RoleGraphqlTestSuite) Test1_FindAllRole() {
	query := `{ findAllRole(input: { search: "", page: 1, pageSize: 10 }) { status message } }`
	result := s.gql(query)
	s.Require().NotEmpty(result)
}

func (s *RoleGraphqlTestSuite) Test2_CreateRole() {
	query := `mutation { createRole(input: { name: "TEST_ROLE_GQL" }) { status message } }`
	result := s.gql(query)
	s.Require().NotEmpty(result)
}

func (s *RoleGraphqlTestSuite) Test3_SmokeQuery() {
	query := `{ __typename }`
	result := s.gql(query)
	s.Require().NotEmpty(result)
}

func TestRoleGraphql(t *testing.T) {
